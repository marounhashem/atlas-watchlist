...
function run(sql, params) { getDb().run(sql, params || []); }
...
function upsertMarketData(symbol, data) {
  run("INSERT INTO market_data (symbol,ts,close,high,low,volume,ema200,vwap,rsi,macd_hist,bias,bias_score,structure,fvg_present,fxssi_long_pct,fxssi_short_pct,fxssi_trapped,ob_absorption,ob_imbalance,ob_large_orders,raw_payload) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    [symbol,Date.now(),data.close,data.high,data.low,data.volume,data.ema200,data.vwap,data.rsi,data.macdHist,data.bias,data.biasScore,data.structure,data.fvgPresent?1:0,data.fxssiLongPct,data.fxssiShortPct,data.fxssiTrapped,data.obAbsorption?1:0,data.obImbalance,data.obLargeOrders?1:0,JSON.stringify(data)]);
}
...
function insertSignal(signal) {
  run("INSERT INTO signals (symbol,ts,direction,score,verdict,entry,sl,tp,rr,session,reasoning) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
    [signal.symbol,Date.now(),signal.direction,signal.score,signal.verdict,signal.entry,signal.sl,signal.tp,signal.rr,signal.session,signal.reasoning]);
  const row = get("SELECT last_insert_rowid() as id");
  return row ? row.id : null;
}

function updateOutcome(signalId, outcome, pnlPct) {
  run("UPDATE signals SET outcome=?,outcome_ts=?,pnl_pct=? WHERE id=?", [outcome,Date.now(),pnlPct,signalId]);
  persist();
}
...
function updateWeights(symbol, weights, winRate, sampleSize) {
  run("INSERT INTO weights (symbol,ts,pine_bias,fxssi_sentiment,order_book,session_quality,min_score_proceed,win_rate,sample_size) VALUES (?,?,?,?,?,?,?,?,?)",
    [symbol,Date.now(),weights.pineBias,weights.fxssiSentiment,weights.orderBook,weights.sessionQuality,weights.minScoreProceed,winRate,sampleSize]);
  persist();
}

function insertLearningLog(entry) {
  run("INSERT INTO learning_log (ts,symbols_analysed,outcomes_used,changes,reasoning) VALUES (?,?,?,?,?)",
    [Date.now(),entry.symbolsAnalysed,entry.outcomesUsed,JSON.stringify(entry.changes),entry.reasoning]);
  persist();
}

module.exports = { init, persist, upsertMarketData, getLatestMarketData, insertSignal, updateOutcome, getOpenSignals, getRecentOutcomes, getWeights, updateWeights, insertLearningLog, getAllSignals, getLearningLog };
