...
`SCORER_VERSION = '20260410.4'`

Changes since 20260401.15:
- **20260410.4** — Intel card full content rendering fix: Added `level_types TEXT` column to `market_intel` table (migration). Pre-analyzed bypass in POST `/api/market-intel` now stores `body.content` (was `body.text`) and writes `level_types` to the new column. Client card renderer: `fullText.split('\n')` with per-line emoji prefix detection BEFORE `esc()` — 📍 blue monospace header, 📝 italic body (prefix sliced), 🔴 red monospace resistance, 🟢 green monospace support, ❌ red invalidations, 🎯 amber catalyst, • amber indented events, ── dim separators. Flat levels row suppressed when content already has 🔴/RESISTANCE. Max-height 500px with overflow-y scroll. Mercato Flow 3 scanner wired into scorer.js. MERCATO badge removed from header.
