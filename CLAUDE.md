...
`SCORER_VERSION = '20260407.1'`

Changes since 20260401.15:
- **20260407.1** — Raised MIN_RR_PROCEED from 2.0 to 2.5 (forces WATCH below 2.5 R:R), rrMax default from 4.0 to 5.0, ATR fallback TP multiplier from 3.0× to 4.0×. Hard kill gate stays at 1.5. New trapped trader hard gate: blocks LONG if buyersInProfitPct <35%, blocks SHORT if sellersInProfitPct <35% (FXSSI profit split). New gravity proximity hard gate: blocks signal if entry within 0.3% of gravity price (SL hunt risk).
