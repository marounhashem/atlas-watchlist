...
`SCORER_VERSION = '20260410.3'`

Changes since 20260401.15:
- **20260412.1** — Intel card UI: body text now shows `content || text || summary` (full raw text instead of truncated summary). Levels display color-coded by type: red `RES:` for resistance, green `SUP:` for support (reads `level_types` JSON from pre-analyzed payloads). Mercato badge removed from dashboard (HTML element, WS handler, page-load fetch all deleted in 071191d).
