...
- **20260410.4** — Intel card full content rendering (client/index.html): Full multiline content field rendered line-by-line with colour coding (📍 header blue, 📝 notes italic, 🔴 RES red, 🟢 SUP green, 🎯 amber, • events amber, ── separators dimmed). Flat levels row hidden when formatted levels already present in content. `esc()` applied per-line AFTER emoji prefix detection (fixes emoji matching after HTML-escape). Max-height 400px with overflow-y scroll. Mercato Flow 3 scanner wired into scorer.js. MERCATO badge removed from header.
