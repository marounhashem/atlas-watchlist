...
- **20260410.4** — Intel card ASCII-marker rendering (client/index.html): Content now uses ASCII section markers (`[SYMBOL | BIAS | REGIME]`, `NOTES:`, `=== KEY LEVELS ===`, `[RES]`, `[SUP]`, `[BULL INV]`, `[BEAR INV]`, `[CATALYST]`, `=== RISK EVENTS ===`, `- ` bullets) instead of emojis (emojis corrupt in sql.js). Renderer splits `content` on `\n`, matches each prefix, applies monospace + colour styling. Flat levels fallback from `key_levels`/`level_types` only shown when content has no `[RES]`/`[SUP]` lines. `level_types TEXT` column added to `market_intel` (migration). Pre-analyzed bypass stores `body.content` and writes `level_types`. Mercato Flow 3 scanner in scorer.js. MERCATO badge removed.
