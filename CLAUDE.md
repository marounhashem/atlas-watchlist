...
- **Economic calendar** polled every 5 minutes from 4 FairEconomy feeds: FF (forex/macro), EE (energy → OILWTI, USDCAD), MM (metals → GOLD, SILVER), CC (crypto → BTCUSD, ETHUSD). Events deduped by title+currency+date with source tracking. Feed-specific symbols always added to affected list. Commodity-aware sentiment: crude inventories higher = bearish OIL. HIGH impact events stored in `economic_events` table. Two-state event risk: **pre-event** (2h window, ×0.75 penalty + cap WATCH) and **post-event** (30min suppression, all signals blocked on affected symbols). Fired events trigger Telegram alert + WebSocket broadcast. Morning brief merges CB meetings + FF events sorted by date.
