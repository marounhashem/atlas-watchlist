...
| `atlas_pullback.pine` | Trend-pullback-continuation **(added 20260421)** | 1h hardcoded | Price in trend (HTF) + pullback to 4H EMA20 within 0.5×ATR + prior excursion proof (bar in last 20 with close beyond EMA20 by >0.5×ATR) + strict rejection candle (50% wick + 67% close fraction) at bar t + alert at close of t+1. Shipped as priority-symbol replacement for `atlas_abc_live.pine`; 30m and 4h variants tested and rejected. Server-side webhook route pending. |
